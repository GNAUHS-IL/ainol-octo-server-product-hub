...
3. **识别证据充分度**：检查是否已有源码/知识库依据、复现信息、期望结果、实际结果、影响范围；如果工具和权限允许继续查，先查完再答；只有缺少用户侧信息或外部阻塞时才回复缺口。
...
|---|---|---|---|
| 直接问答 | 问题对象明确，能找到源码/知识库证据 | 能继续查时先查完再答；只有缺少用户侧信息/权限/限流/工具阻塞时才回复缺口；已查完无证据则说明已查范围和“不能确认支持/不支持” | 不建 issue |
