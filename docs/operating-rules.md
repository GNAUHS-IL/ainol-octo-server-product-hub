...
- 答不上来就明确说“我目前不确定”，不得为了完整性编造结论、路径、行号或产品状态。
- 知识库没有覆盖时，不能直接说“不支持”或“没有实现”；必须说明“现有知识库未覆盖，需要补查 <目录/文件/模块>”。
- 补查顺序：先查九大知识库和 `knowledge/10-cross-module-quickref.md`，再查 `docs/source-audit/` 索引，最后回到目标仓源码 grep/读取最小证据片段。
- 补查后仍找不到证据时，对外口径是“不确定，当前证据不足以支撑结论”，并说明下一步要补查谁或哪个模块。
- 不确定问题如果只是问答，不建 issue；如果暴露知识库缺口、文档缺失或需要持续追踪，按 `type/feature` 或 `status/blocked` 归档，并在正文/comment 写明“证据待补查”。

## 状态转达真实性红线

- 对外转达必须使用真实状态，不得把“已修复、没复现、不做、wontfix、duplicate、invalid、blocked”混用。
- `status/done` 只能在有完成证据时说“已完成/已修复”。
- 没复现只能说“当前未复现/证据不足”，不能说“已修复”。
- `status/wontfix` 只能说“确认不处理/暂不处理，并说明原因”，不能说“已修复”。
- `status/duplicate` 只能说“重复，转到原 issue 跟进”，不能说“已修复”。
- `status/invalid` 只能说“无法成立为有效工作项/事实不成立”，不能说“已修复”。
- `status/blocked` 只能说“当前被阻塞，需要补证据/权限/人工复核确认”，不能说“已完成”。
