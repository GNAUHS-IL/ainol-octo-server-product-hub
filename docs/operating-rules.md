...
- 有实质状态变化才同步；无变化只写日志。
- 同步内容优先给结论、影响、证据和下一步，不展开内部调试过程。
- 用户交互、查证、追问、建单和不确定口径统一按 `docs/user-interaction-decision-protocol.md` 执行。

## 正式答复质量

- 对源码/产品事实类问题，能查就先查完；不能把“需要补查”当正式答案。
- 源码/产品结论必须引用真实源码，单个引用跨度默认不超过 15 行。
- 已完成合理查证但无证据时，说明已查范围、未找到支撑证据、不能确认支持/不支持。
- 只有缺少用户侧不可替代信息、外部权限/限流/工具阻塞，或用户要求进度时，才在未形成结论前回复缺口。
