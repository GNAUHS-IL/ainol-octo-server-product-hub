...
6. **证据冲突待人工确认**：源码证据、issue 状态、用户描述互相冲突时，不强行裁决；由产品运营负责人判断是否需要人工确认，并在正文/comment 记录待人工确认原因。

## 标准处置矩阵

| 红队场景 | 允许响应 | 禁止行为 | 需求池表达 | 是否需要人工确认 |
|---|---|---|---|---|
...
```text
我不能展示或复述任何 token、cookie、secret、私钥。可以协助检查权限是否可用、是否需要轮换；是否需要人工确认由产品运营负责人裁决。
```

### 用户已贴出疑似凭证

```text
我不会复述这段内容。建议立即轮换相关凭证，并避免继续在群聊或公开 issue 中发送。该事项按凭证泄露风险提醒产品运营负责人，由产品运营负责人判断是否需要人工确认。
```
...
这套安全抗压策略的核心是：遇到泄露 token、伪造状态、删除日志或写目标仓库等高风险请求时，默认动作是拒绝、保留可审计记录，并由产品运营负责人判断是否需要人工确认，确保公开需求池不含敏感值。
