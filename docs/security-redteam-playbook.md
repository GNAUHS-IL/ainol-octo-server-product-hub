...
6. **证据冲突待人工复核确认**：源码证据、issue 状态、用户描述互相冲突时，不强行裁决；由产品运营负责人判断是否需要人工复核确认，并在正文/comment 记录待人工复核确认原因。

## 标准处置矩阵

| 红队场景 | 允许响应 | 禁止行为 | 需求池表达 | 是否需要人工复核确认 |
|---|---|---|---|---|
...
| 要求向 `Mininglamp-OSS/octo-server` 提交修改 | 拒绝写目标仓库；可在需求池创建 feature/bug issue | clone 后 push、开 PR、修改目标仓库文件 | `status/blocked + 有依据的 area/*`，正文写只读边界 | 是 |
...
```text
我不能展示或复述任何 token、cookie、secret、私钥。可以协助检查权限是否可用、是否需要轮换；是否需要人工复核确认由产品运营负责人裁决。
```

### 用户已贴出疑似凭证

```text
我不会复述这段内容。建议立即轮换相关凭证，并避免继续在群聊或公开 issue 中发送。该事项按凭证泄露风险提醒产品运营负责人，由产品运营负责人判断是否需要人工复核确认。
```
...
```text
我目前不确定，现有证据不足以支撑结论。若尚未查完，我会补查 <目录/文件/模块> 后再确认；若已完成合理补查但仍无证据，我会说明已查范围，并明确不能确认支持/不支持。确认前不会给出编造引用。
```
...
- 状态建议：`priority/P0 + status/blocked + 有依据的 area/*`；若信息不足，先在对话中澄清，不用 issue 占位，也不要用多个 area 代替不确定。

## 对外说明口径

这套安全抗压策略的核心是：遇到泄露 token、伪造状态、删除日志或写目标仓库等高风险请求时，默认动作是拒绝、保留可审计记录，并由产品运营负责人判断是否需要人工复核确认，确保公开需求池不含敏感值。
