...
每个 issue 至少有一个 `area/*`。跨领域问题只有在每个领域都有明确依据时，才可以打多个 `area/*`；`area/*` 不是越多越好。
...
`knowledge/10-cross-module-quickref.md` 是跨模块速查页，不对应单独 label。跨模块问题按证据逐项判断：只给有明确事实、源码引用、知识库映射或验收范围支撑的领域打 `area/*`；弱相关、推测性或“可能涉及”的领域写入 issue 正文/comment 的“待核验范围”，不打 label。

## 4.5 `area/*` 多标签判定规则

`area/*` 的作用是把 issue 路由到可核验的源码知识领域，不用于罗列所有可能相关模块。

判定顺序：

1. **先定主 area**：根据 issue 的核心对象/用户问题，选择一个最主要的 `area/*`。
2. **再定关联 area**：只有当 issue 事实、复现步骤、源码证据、知识库映射或 PRD 验收范围明确覆盖另一个领域时，才补充第二个 `area/*`。
3. **不打弱相关 area**：仅因“可能经过某链路”“未来可能影响某模块”“经验上可能相关”而产生的领域，不打 label，只写在正文/comment。
4. **用 `area/unknown` 兜底**：暂不能判断时先用 `area/unknown`，后续拿到证据后替换为具体 area；不要用多个 area 代替不确定。

多 `area/*` 的证据门槛：

| 可以追加 area 的依据 | 不应追加 area 的情况 |
|---|---|
| 用户描述明确点名该领域，例如 token、权限、IM 消息、配置、构建 | 只是实现链路可能经过该模块 |
| 源码引用或知识库条目证明该领域是问题发生点或验收对象 | 只是为了“覆盖全面” |
| PRD/验收标准明确要求该领域产生用户可见变化 | 只是后续排查可能会看它 |
| 现有 issue/comment 已有可核验证据指向该领域 | 没有证据，只是推测 |

示例：

- “Bot API 报错太笼统”默认使用 `area/bot-agent + area/api-error`。
- 只有当描述明确为“token 失效时报错太笼统”时，才追加 `area/auth`。
- 只有当描述明确为“权限不足时报错太笼统”时，才追加 `area/rbac`。
- 只是“Bot 发消息可能经过 IM 链路”时，不追加 `area/im`。
...
6. 每个 issue 至少有一个 `area/*`，但必须有依据；默认主 area + 必要且有证据的关联 area。
...
2. **足够贴合产品**：`area/*` 与九大知识库一一对应，且按证据最小化使用，便于把反馈落回源码证据和知识库。
