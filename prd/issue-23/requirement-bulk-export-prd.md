...
- 导出结果至少包含标题、编号、状态、负责人、创建时间、最近更新时间和摘要。
- 导出内容仅包含当前用户有权查看的需求单，不泄露无权限、私密或跨 Space / 群 / Thread / Bot 范围的数据。
- 对导出格式、字段口径、空结果、超量、失败、敏感内容、限流和审计建立清晰产品口径。
...
- 边界场景：负责人为空、摘要缺失、标题过长、状态不明确、多个标签冲突或需求已关闭/重开时，应按当前真实状态展示，并提示待确认项。
...
- 需求描述：导出文件应采用常见、可打开、便于编辑或统计的列表格式，并在导出完成后提供清晰的下载入口。
- 业务规则：首版导出格式应满足评审、周报和复盘的表格化整理需求；文件名、导出时间、筛选条件摘要和数据范围应便于识别。
...
- 业务规则：导出前或导出中应让用户理解结果规模和限制；超出边界时应建议缩小时间范围、状态、创建人或标签条件。
...
- AC-02：当筛选结果不为空且在可导出范围内时，用户能一键导出需求单列表，并获得可下载文件。
- AC-03：导出结果至少包含标题、编号、状态、负责人、创建时间、最近更新时间和摘要，字段名称清晰可读。
- AC-04：导出结果只包含当前用户有权查看的需求单和字段内容；无权、私密、跨 Space / 群 / Thread / Bot 的内容不出现在导出文件中。
- AC-05：当筛选结果为空时，用户看到“无可导出结果”及调整筛选条件的建议。
- AC-06：当导出范围过大、频率过高或重复触发时，用户看到限制原因和缩小范围或稍后重试的建议。
- AC-07：当导出中发生权限变化、需求状态变化、文件生成失败或下载入口失效时，用户看到真实状态，不误报为完整成功。
...
- AC-11：产品说明覆盖筛选条件、导出字段、导出格式、权限过滤、空结果、超量、失败、脱敏、限流和审计口径。
...
1. [待确认] 首版导出格式采用单一表格格式，还是同时提供表格与纯文本/数据格式。
2. [待确认] “负责人”字段首版取当前处理人、产品运营负责人、issue assignee，还是允许为空并展示待分配。
3. [待确认] 首版导出数量上限和导出文件有效期由产品运营负责人确认。
...
- 限流 / 防滥用：筛选、导出和下载需要频率、数量、范围和文件大小边界，避免批量抓取、枚举无权对象或对服务造成压力。
...
- Label 完整性：通过；当前远端包含 `type/feature`、`priority/P2`、`status/prd-drafting`、`area/unknown`。
- 状态真实性：当前 issue 仍为 `status/prd-drafting`；PRD 文件提交、评论回填和核验通过后应更新为唯一 `status/reviewing`。
- 风险提醒 / 待人工确认：无阻塞级风险；建议 Review 重点关注批量导出权限过滤、无权对象防枚举、导出范围上限、敏感内容脱敏、导出文件有效期、限流和审计。
